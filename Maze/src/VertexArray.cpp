...
{
	const auto color = sf::Color(150, 150, 150);

	constexpr int line_size = 1.f;
	...
	if (vertical)
	{
		vertices[0].position = sf::Vector2f(x, 0.0f);
		vertices[1].position = sf::Vector2f(x + line_size, 0.0f);
		vertices[2].position = sf::Vector2f(x + line_size, max_strip_size);
		vertices[3].position = sf::Vector2f(x, max_strip_size);
	}
	else
	{
		vertices[0].position = sf::Vector2f(0.0f, y);
		vertices[1].position = sf::Vector2f(width, y);
		vertices[2].position = sf::Vector2f(width, y + line_size);
		vertices[3].position = sf::Vector2f(0.0f, y + line_size);
	}
...
{
	constexpr long nodes_size = SIZE_X * SIZE_Y * 4;
...
{
	constexpr long grid_size = SIZE_X * SIZE_Y * 4;
