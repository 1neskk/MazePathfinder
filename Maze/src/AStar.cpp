...
void AStar::reconstructPath(Node* finish_node, const Node* start_node)
{
...
std::vector<Node*> AStar::getNeighbourNodes(std::vector<Node>& nodes, Node* current_node, bool diagonal) const
{
...
int AStar::calculateDistance(const Node& n1, const Node& n2) const
{
...
int AStar::getIndex(const int X, const int Y) const
{
	return Y * SIZE_X + X;
}

bool AStar::startAStar(std::vector<Node>& nodes, float& pathfind_time, bool diagonal, Node* start_node, const Node* end_node) const
{
